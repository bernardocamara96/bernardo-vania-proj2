...
test("User not validated: invalid email", async ()=>{
   try{
   const response = await register.validateUser("fjose", "12345", "ddscd" ,
   "Daniefla", "Mefndes", "989061551");
...
test("User not validated: email allready exists", async ()=> {
   try{
   const response = await register.validateUser("1234", "1234", "vafnia@gmail.com",
   "Daniefla", "Menfdes", "912361551");
...
test("User not validated: username allready exists", async ()=> {
   try{
   const response = await register.validateUser("vania1", "12345", "daniddfe@gmail.com",
   "Dafnifedla", "Mfenfddes", "985236521");
       expect(response).toBe(409);
   }catch(error){
      expect(error).toBeUndefined();
   }
...
test("User not validated: invalid phone", async ()=> {
   ...
   const response = await register.validateUser("1234", "1234", "danerdie@gmail.com",
   "Dasdfniela", "Mfffendes", "0780");
      expect(response).toBe(400);
   }catch(error){
      expect(error).toBeUndefined();
   }
